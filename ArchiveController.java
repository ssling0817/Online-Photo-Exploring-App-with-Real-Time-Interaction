...
public class ArchiveController {
    ...
    void nextPressed(ActionEvent event)throws Exception {
		if((pagePhotoStatus+1+16*(totalpagenum-pagenum)<=photoNum)&&pagenum!=totalpagenum)
		{
			...
			totalPageLabel.setText(String.valueOf(totalpagenum));
				...
				//0624
				image=sqlcon2.SelectArchiveTable(account,pagePhotoStatus,photoIndex);
				int num;
				for(int i=0;i<16;i++)
				{
					num=i+1;
    ...
    void imageView1Clicked(MouseEvent event) throws Exception{ 
		ArchiveInfo photoInfo=new ArchiveInfo();
	...
	public void setStage(Stage stage, HomepageController home,Archive f) throws Exception
	{
		this.homepagec=home;
		this.stage=stage;
		this.a=f;
		account=homepagec.getAccount();
		//following=sqlcon2.followingTable(account);
		//System.out.println("fc: "+following[0]);
		int num;
		int picNum=sqlcon2.getArchiveCount(account);
		photoNum=picNum;
		photoIndex=photoNum%16;
		...
		currentPageLabel.setText(String.valueOf(pagenum));
		totalPageLabel.setText(String.valueOf(totalpagenum));
		//System.out.println("archivecontroller: n "+photoNum);
		if(picNum>=16)
			image = new Image[16];
		else
