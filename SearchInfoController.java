...
public class SearchInfoController {
    ...
    void userLabelClicked(MouseEvent event)throws Exception {
		if(myAccount.compareTo(following)!=0)
		{
			...
			targetHome.showWindow(homepagec,this,myAccount,following,search,num,pagePhotoStatus,photoIndex);
    ...
    void returnClicked(ActionEvent event) throws Exception{
		s.getSearch().showWindow(homepagec);
		((Node)(event.getSource())).getScene().getWindow().hide();
	...
	public void setStage(Stage stage, HomepageController home,int num,SearchController s,int pagePhotoStatus,int photoIndex,String search,SearchInfo si)throws Exception
	{
		this.homepagec=home;
		this.stage=stage;
		this.num=num;
		...
		myAccount=account;
		//getImage
		images=sqlcon.SelectWordTable(search,pagePhotoStatus,photoIndex);//
		photosearch=sqlcon.SelectWordSearchTable(search,pagePhotoStatus,photoIndex);//
		accountsearch=sqlcon.SelectWordAccountTable(search,pagePhotoStatus,photoIndex);//
		following=accountsearch[num];

		date=sqlcon.SelectWordDateTable(search,pagePhotoStatus,photoIndex);
		image=images[num];
